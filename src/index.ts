import { resolve, extname, join as pathJoin } from 'node:path';
import { writeFileSync, mkdirSync, watch, readFileSync } from 'node:fs';
...
function getCliVersion(): string {
  try {
    ...
    const pkg = JSON.parse(readFileSync(pathJoin(thisDir, '..', 'package.json'), 'utf-8'));
...
function runTestAutogenCommand(
  projectDir: string,
  stack: DetectedStack,
  opts: Record<string, string | boolean>,
): void {
  ...
  console.log('');
  console.log(
    `  ${pc.bold(pc.magenta('forge-ai-init test-autogen'))} — Required Test Generation`,
  );
  console.log('');
  ...
  if (result.bypassed) {
    console.log(
      `  ${pc.yellow('Bypassed')} until ${result.bypassExpiresAt} — ${result.bypassReason}`,
    );
    console.log('');
  }
  ...
  if (result.created.length > 0) {
    ...
    for (const file of result.created.slice(0, 20)) {
      console.log(`    ${pc.green('+')} ${file}`);
    }
    if (result.created.length > 20) {
      console.log(
        `    ${pc.dim(`... and ${result.created.length - 20} more`)}`,
      );
    }
    console.log('');
  }
  ...
  if (result.missing.length > 0) {
    ...
    for (const file of result.missing.slice(0, 20)) {
      console.log(`    ${pc.red('✗')} ${file}`);
    }
    if (result.missing.length > 20) {
      console.log(
        `    ${pc.dim(`... and ${result.missing.length - 20} more`)}`,
      );
    }
    console.log('');
  }
...
async function main(): Promise<void> {
  ...
  if (opts['command'] === 'test-autogen') {
    runTestAutogenCommand(projectDir, stack, opts);
    return;
  }
